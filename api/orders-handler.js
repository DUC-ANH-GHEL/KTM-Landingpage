import{neon as ae}from"@neondatabase/serverless";import Nt from"crypto";const r=ae(process.env.DATABASE_URL);let vt=!1,Lt=0;const ue=10*60*1e3;let Rt=0;const de=10*60*1e3,dt=3;function St(t){return t?String(t).replace(/[^0-9]/g,""):""}function Tt(t,e=!1){if(t==null)return e;const n=String(t).trim().toLowerCase();return n==="1"||n==="true"||n==="yes"||n==="y"||n==="on"?!0:n==="0"||n==="false"||n==="no"||n==="n"||n==="off"?!1:e}function Wt(t,e=null){const n=Number(t);return Number.isFinite(n)?Math.trunc(n):e}function Ht(t){const n=String(t??"").trim().match(/^([0-9]{4})-([0-9]{2})$/);if(!n)return null;const p=Number(n[1]),m=Number(n[2]);if(!Number.isFinite(p)||!Number.isFinite(m)||m<1||m>12)return null;const s=String(m).padStart(2,"0");return`${p}-${s}-01`}function Xt(t){if(t==null||t==="")return 0;const e=String(t).replace(/[^0-9]/g,"");return e?Number(e):0}function ce(t){if(!t)return null;const e=String(t),n=e.toLowerCase();if(n.includes("freeship")||n.includes("free ship")||n.includes("mi\u1EC5n ph\xED ship")||n.includes("mien phi ship")||n.includes("mi\u1EC5n ph\xED v\u1EADn chuy\u1EC3n")||n.includes("mien phi van chuyen"))return 0;const p=e.match(/(?:ship|vận\s*chuyển|van\s*chuyen|\bvc\b)\s*[:=\-]?\s*([0-9][0-9\.,\s]*)(?:\s*(k|nghìn|nghin|tr|triệu|trieu|m))?/i);if(!p)return null;const m=String(p[1]||"").replace(/[^0-9]/g,"");if(!m)return null;let s=Number(m);const N=String(p[2]||"").toLowerCase();return N==="k"||N.startsWith("ngh")?s*=1e3:(N==="tr"||N.startsWith("tri")||N==="m")&&(s*=1e6),Number.isFinite(s)?s:null}const me=1.64;function Ee(t){const e=typeof t=="number"?t:parseFloat(String(t??"").trim());return Number.isFinite(e)?Math.max(0,Math.min(100,e)):me}function _e(t){const e=String(t??"").trim().toLowerCase();return e==="cancelled"?"canceled":e}async function pe(){if(!vt){await r`
  ...
  `,await r`CREATE INDEX IF NOT EXISTS order_items_order_id_idx ON order_items(order_id)`,await r`CREATE INDEX IF NOT EXISTS order_items_product_id_idx ON order_items(product_id)`,await r`CREATE INDEX IF NOT EXISTS order_items_created_at_idx ON order_items(created_at)`,await r`ALTER TABLE order_items ADD COLUMN IF NOT EXISTS unit_price INTEGER`,await r`ALTER TABLE order_items ADD COLUMN IF NOT EXISTS variant TEXT`,await r`ALTER TABLE order_items ADD COLUMN IF NOT EXISTS variant_json JSONB`;try{await r`ALTER TABLE order_items
    ...
    `}catch{}for(const t of["customer_name","phone","address"])try{await r(`ALTER TABLE orders ALTER COLUMN ${t} DROP NOT NULL`)}catch{}try{await r`ALTER TABLE orders
      ...
      ON DELETE SET NULL
    `}catch{}vt=!0}}async function he(){const t=Date.now();Lt&&t-Lt<ue||(Lt=t,await r`
    UPDATE orders
  ...
  `)[0]}async function Ne(t,e){if(e.setHeader("Access-Control-Allow-Origin","*"),e.setHeader("Access-Control-Allow-Methods","GET, POST, PUT, DELETE, OPTIONS"),e.setHeader("Access-Control-Allow-Headers","Content-Type"),t.method==="OPTIONS")return e.status(200).end();const n=t.query.id||t.body?.id,p=n!=null?String(n):null,m=t.query.resource;try{const s=Tt(t.query?.debug,!1),N=s?Date.now():0;await pe();const M=s?Date.now():0;await le();const ot=s?Date.now():0;if(m==="customers"){if(t.method==="GET"){const h=St(t.query.phone);if(!h)return e.status(400).json({error:"phone is required"});const y=await r`
        ...
        `;if(!D.length)return e.status(200).json({exists:!1});const H={name:D[0].customer_name||null,phone:h,address:D[0].address||null};try{const I=Nt.randomUUID(),O=await r`
          ...
          `;return e.status(200).json({exists:!0,customer:O[0]})}catch{const I=await r`
            ...
            WHERE phone = ${h}
            LIMIT 1
          `;return I.length?e.status(200).json({exists:!0,customer:I[0]}):e.status(200).json({exists:!1})}}return e.status(405).json({error:"Method not allowed"})}if(t.method==="GET"){let h=null;if(m==="stats"){const a=String(t.query.month??"").trim(),d=Ht(a);if(!d)return e.status(400).json({error:"month is required (YYYY-MM)"});const B=Ee(t.query.ship_percent??t.query.shipPercent),st=Tt(t.query.meta,!1);let c=[];try{c=await r`SELECT id, name, code, price, note, commission_percent, variants FROM products`}catch{c=[]}const f=new Map;for(const o of Array.isArray(c)?c:[]){const i=String(o?.id??"").trim();i&&f.set(i,o)}const et=o=>(Array.isArray(o)?o:[]).map(L=>{const U=String(L?.name||"").trim();if(!U)return null;const v=(Array.isArray(L?.options)?L.options:[]).map(R=>{const g=String(R?.label||"").trim();if(!g)return null;const S=R?.price,C=S==null||S===""?NaN:typeof S=="number"?S:typeof S=="string"?Xt(S):Number(S),Y=Number.isFinite(C)?Math.trunc(C):null,G=R?.price_delta??R?.priceDelta??null,$=Number(G),J=Number.isFinite($)?Math.trunc($):0;return{label:g,price:Y,price_delta:J}}).filter(Boolean);return{name:U,options:v}}).filter(Boolean),mt=o=>{const i=String(o??"").trim();return i&&f.get(i)||null},Bt=o=>{const i=o?.unit_price??o?.unitPrice,L=i==null||i===""?NaN:Number(i);if(Number.isFinite(L))return Math.max(0,Math.trunc(L));const U=String(o?.product_id||"").trim(),v=mt(U),R=Xt(v?.price),g=o?.variant_json??o?.variantJson,S=g&&typeof g=="object"?g:null;if(!S)return R;const C=et(v?.variants);if(!C.length)return R;let Y=R;for(const G of C){const $=String(G?.name||"").trim(),J=String(S?.[$]||"").trim();if(!$||!J)continue;const at=(Array.isArray(G.options)?G.options:[]).find(ut=>String(ut?.label||"").trim()===J);if(!at)continue;if(Number.isFinite(Number(at.price))){Y=Math.max(0,Math.trunc(Number(at.price)));continue}const Et=Number(at.price_delta);Number.isFinite(Et)&&(Y+=Math.trunc(Et))}return Y},At=5,Ct=new Map((Array.isArray(c)?c:[]).map(o=>{const i=o?.commission_percent??o?.commissionPercent,L=Number(i),U=Number.isFinite(L)?Math.max(0,Math.min(100,L)):At;return[String(o?.id),U]})),Vt=o=>o.customer_id||o.phone||"unknown",Yt=o=>{const i=Array.isArray(o)?o:[];let L=!1,U=0;for(const v of i){const R=String(v?.product_id||"").trim(),g=mt(R),S=v?.product_note??g?.note??null,C=ce(S);C!=null&&(L=!0,C>U&&(U=C))}return{found:L,fee:U}},Gt=s?Date.now():0,Dt=await r(`
          SELECT
            o.id,
            o.parent_order_id,
            o.split_seq,
          ...
          ORDER BY o.created_at DESC
        `,[d]),Ft=s?Date.now():0,$t=(Array.isArray(Dt)?Dt:[]).map(gt),V={draft:0,pending:0,processing:0,done:0,paid:0,canceled:0,other:0};let it=0,ht=0,Ot=0,ft=0,Jt=0,wt=0,Mt=0,jt=0,qt=0;const It=new Map,_t=new Map,yt=new Map;for(const o of $t){const i=_e(o?.status),v=i==="canceled"||i==="draft",R=Array.isArray(o?.items)?o.items:[];let g=0,S=0,C=0;for(const K of R){const w=Number(K?.quantity||0)||0,rt=Bt(K),nt=w*rt,W=String(K?.product_id||""),se=Ct.has(W)?Ct.get(W):At,ie=(Number(se)||0)/100;if(g+=w,S+=nt,C+=nt*ie,!v){const pt=K?.product_id||"unknown",Ut=mt(pt),lt=It.get(pt)||{product_id:pt,product_name:Ut?.name||"\u2014",product_code:Ut?.code||"",orders:0,quantity:0,revenue:0};lt.orders+=1,lt.quantity+=w,lt.revenue+=nt,It.set(pt,lt)}}const Y=Yt(R),G=Number(o?.adjustment_amount??0)||0,$=S+(Y.found?Y.fee:0)+G,J=S+G,at=!Y.found&&B>0&&S>0?S*B/100:0,Et=S>0?C/S:At/100,ut=C+G*Et-at*Et,oe=i==="done"||i==="paid";if(i==="draft"?V.draft+=1:i==="pending"?V.pending+=1:i==="processing"?V.processing+=1:i==="canceled"?V.canceled+=1:i==="paid"?(V.paid+=1,V.done+=1,ft+=$,wt+=J,qt+=ut):i==="done"?(V.done+=1,ft+=$,wt+=J):V.other+=1,i!=="paid"&&i!=="canceled"&&i!=="draft"&&(jt+=ut),!v){it+=1,ht+=g,Ot+=$,Jt+=J,Mt+=ut;const K=Vt(o),w=_t.get(K)||{key:K,customer_name:o.customer_name||"",phone:o.phone||"",orders:0,quantity:0,revenue:0};w.orders+=1,w.quantity+=g,w.revenue+=$,!w.customer_name&&o.customer_name&&(w.customer_name=o.customer_name),!w.phone&&o.phone&&(w.phone=o.phone),_t.set(K,w);const rt=o.created_at?new Date(o.created_at):null;if(rt&&!Number.isNaN(rt.getTime())){const nt=`${rt.getFullYear()}-${String(rt.getMonth()+1).padStart(2,"0")}-${String(rt.getDate()).padStart(2,"0")}`,W=yt.get(nt)||{day:nt,orders:0,quantity:0,revenue:0,doneOrders:0,doneRevenue:0,commission:0};W.orders+=1,W.quantity+=g,W.revenue+=$,W.commission+=ut,oe&&(W.doneOrders+=1,W.doneRevenue+=$),yt.set(nt,W)}}}const Kt=Array.from(It.values()).sort((o,i)=>i.revenue-o.revenue),zt=Array.from(_t.values()).sort((o,i)=>i.revenue-o.revenue),kt=Array.from(yt.values()).sort((o,i)=>o.day.localeCompare(i.day)),Qt=_t.size,Zt=it?Math.round(Ot/it):0,te=it?ht/it:0,ee=Math.round(jt),re=Math.round(Mt),ne=Math.round(qt),bt={statusCounts:V,activeOrders:it,totalQty:ht,totalRevenue:Ot,doneRevenue:ft,tempCommission:ee,tempCommissionAll:re,commissionPaid:ne,products:Kt,customers:zt,days:kt,uniqueCustomers:Qt,avgOrderValue:Zt,avgQtyPerOrder:te};return st?e.status(200).json({stats:bt,meta:{month:a,ship_percent:B,ordersCount:$t.length,...s?{timingsMs:{schema:M-N,cleanup:ot-M,autoAdvance:h,query:Ft-Gt,total:Ft-N}}:{}}}):e.status(200).json(bt)}if(n){const d=await r(`
          SELECT
            o.id,
            o.parent_order_id,
            o.split_seq,
          ...
          WHERE o.id = $1
          GROUP BY o.id, c.id, p0.id
        `,[n]);return d.length?e.status(200).json(gt(d[0])):e.status(404).json({error:"Order not found"})}const{month:y}=t.query,D=y?Ht(y):null,H=t.query.search??t.query.q??"",I=String(H??"").trim(),O=I.replace(/[^0-9]+/g,""),ct=Tt(t.query.includeItems??t.query.items??t.query.include_items,!0),j=Tt(t.query.meta,!1),u=Wt(t.query.limit,null),A=Math.max(0,Wt(t.query.offset,0)??0),X=s&&j,z=String(t.query.overdue||"").trim()==="1"||String(t.query.overdue||"").trim().toLowerCase()==="true",x=String(t.query.draftExpiring||t.query.draft_expiring||"").trim()==="1"||String(t.query.draftExpiring||t.query.draft_expiring||"").trim().toLowerCase()==="true",k=t.query.days??t.query.overdueDays??3;let F=Number(k);(!Number.isFinite(F)||F<0)&&(F=3),F=Math.trunc(F);const Q=t.query.remainingDays??t.query.remaining_days??3;let T=Number(Q);(!Number.isFinite(T)||T<0)&&(T=3),T=Math.trunc(T),T>dt&&(T=dt);const q=Math.max(0,dt-T);let l=`
        SELECT
          o.id,
          o.parent_order_id,
          o.split_seq,
      ...
      `;const E=[];if(I?(l+=" WHERE (",l+=" (c.name ILIKE $1 OR o.customer_name ILIKE $1)",E.push(`%${I}%`),O&&(O.length>=9&&(l+=` OR (c.phone = $${E.length+1} OR o.phone = $${E.length+1})`,E.push(O)),l+=` OR (c.phone LIKE $${E.length+1} OR o.phone LIKE $${E.length+1})`,E.push(`%${O}%`)),l+=" )"):x?(l+=" WHERE o.status = 'draft' AND o.created_at IS NOT NULL",l+=" AND o.created_at > (NOW() - ($1::int * INTERVAL '1 day'))",E.push(dt),q>0&&(l+=" AND o.created_at <= (NOW() - ($2::int * INTERVAL '1 day'))",E.push(q))):z?(l+=" WHERE o.status = 'pending' AND o.created_at <= (NOW() - ($1::int * INTERVAL '1 day'))",E.push(F)):D?(l+=" WHERE o.created_at >= $1::date AND o.created_at < ($1::date + INTERVAL '1 month')",E.push(D)):y&&(l+=" WHERE TO_CHAR(o.created_at, 'YYYY-MM') = $1",E.push(y)),l+=" GROUP BY o.id, c.id, p0.id ORDER BY o.created_at DESC",Number.isFinite(u)&&u>0){const a=j?u+1:u;l+=` LIMIT $${E.length+1}`,E.push(a),A>0&&(l+=` OFFSET $${E.length+1}`,E.push(A))}else A>0&&(l+=` OFFSET $${E.length+1}`,E.push(A));if(!ct){let a=`
        ...
        `,et=Number(f?.[0]?.next_seq??2);a=Number.isFinite(et)?Math.trunc(et):2,a<2&&(a=2)}const B=(P==null?await r`
          ...
          `:await r`
            ...
            VALUES (${Q.id}, ${_}, ${a}, ${q.product_id}, ${q.quantity}, ${O}, NOW(), ${E}, ${Z}, ${tt}, ${P}::timestamp)
            RETURNING id
