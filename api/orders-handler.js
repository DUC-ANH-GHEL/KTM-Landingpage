  ...
  `)[0]}async function Ne(t,e){if(e.setHeader("Access-Control-Allow-Origin","*"),e.setHeader("Access-Control-Allow-Methods","GET, POST, PUT, DELETE, OPTIONS"),e.setHeader("Access-Control-Allow-Headers","Content-Type"),t.method==="OPTIONS")return e.status(200).end();const n=t.query.id||t.body?.id,p=n!=null?String(n):null,m=t.query.resource;try{const s=Tt(t.query?.debug,!1),N=s?Date.now():0;await pe();const M=s?Date.now():0;await le();const ot=s?Date.now():0;if(m==="customers"){if(t.method==="GET"){const h=St(t.query.phone);if(!h)return e.status(400).json({error:"phone is required"});const y=await r`
        ...
        `;if(!D.length)return e.status(200).json({exists:!1});const H={name:D[0].customer_name||null,phone:h,address:D[0].address||null};try{const I=Nt.randomUUID(),O=await r`
          ...
          `;return e.status(200).json({exists:!0,customer:O[0]})}catch{const I=await r`
            ...
            WHERE phone = ${h}
            LIMIT 1
          `;return I.length?e.status(200).json({exists:!0,customer:I[0]}):e.status(200).json({exists:!1})}}return e.status(405).json({error:"Method not allowed"})}if(t.method==="GET"){let h=null;if(m==="stats"){const i=String(t.query.month??"").trim(),d=Ht(i);if(!d)return e.status(400).json({error:"month is required (YYYY-MM)"});const B=Ee(t.query.ship_percent??t.query.shipPercent),st=Tt(t.query.meta,!1);let c=[];try{c=await r`SELECT id, name, code, price, note, commission_percent, variants FROM products`}catch{c=[]}const f=new Map;for(const o of Array.isArray(c)?c:[]){const a=String(o?.id??"").trim();a&&f.set(a,o)}const et=o=>(Array.isArray(o)?o:[]).map(L=>{const U=String(L?.name||"").trim();if(!U)return null;const v=(Array.isArray(L?.options)?L.options:[]).map(R=>{const g=String(R?.label||"").trim();if(!g)return null;const S=R?.price,C=S==null||S===""?NaN:typeof S=="number"?S:typeof S=="string"?Xt(S):Number(S),Y=Number.isFinite(C)?Math.trunc(C):null,G=R?.price_delta??R?.priceDelta??null,$=Number(G),J=Number.isFinite($)?Math.trunc($):0;return{label:g,price:Y,price_delta:J}}).filter(Boolean);return{name:U,options:v}}).filter(Boolean),mt=o=>{const a=String(o??"").trim();return a&&f.get(a)||null},Bt=o=>{const a=o?.unit_price??o?.unitPrice,L=a==null||a===""?NaN:Number(a);if(Number.isFinite(L))return Math.max(0,Math.trunc(L));const U=String(o?.product_id||"").trim(),v=mt(U),R=Xt(v?.price),g=o?.variant_json??o?.variantJson,S=g&&typeof g=="object"?g:null;if(!S)return R;const C=et(v?.variants);if(!C.length)return R;let Y=R;for(const G of C){const $=String(G?.name||"").trim(),J=String(S?.[$]||"").trim();if(!$||!J)continue;const at=(Array.isArray(G.options)?G.options:[]).find(ut=>String(ut?.label||"").trim()===J);if(!at)continue;if(Number.isFinite(Number(at.price))){Y=Math.max(0,Math.trunc(Number(at.price)));continue}const Et=Number(at.price_delta);Number.isFinite(Et)&&(Y+=Math.trunc(Et))}return Y},At=5,Ct=new Map((Array.isArray(c)?c:[]).map(o=>{const a=o?.commission_percent??o?.commissionPercent,L=Number(a),U=Number.isFinite(L)?Math.max(0,Math.min(100,L)):At;return[String(o?.id),U]})),Vt=o=>o.customer_id||o.phone||"unknown",Yt=o=>{const a=Array.isArray(o)?o:[];let L=!1,U=0;for(const v of a){const R=String(v?.product_id||"").trim(),g=mt(R),S=v?.product_note??g?.note??null,C=ce(S);C!=null&&(L=!0,C>U&&(U=C))}return{found:L,fee:U}},Gt=s?Date.now():0,Dt=await r(`
          SELECT
            o.id,
            o.parent_order_id,
            o.split_seq,
