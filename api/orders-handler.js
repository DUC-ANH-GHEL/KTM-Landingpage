  ...
  `)[0]}async function le(t,e){if(e.setHeader("Access-Control-Allow-Origin","*"),e.setHeader("Access-Control-Allow-Methods","GET, POST, PUT, DELETE, OPTIONS"),e.setHeader("Access-Control-Allow-Headers","Content-Type"),t.method==="OPTIONS")return e.status(200).end();const n=t.query.id||t.body?.id,p=n!=null?String(n):null,m=t.query.resource;try{const s=Tt(t.query?.debug,!1),N=s?Date.now():0;await _e();const M=s?Date.now():0;await pe();const ot=s?Date.now():0;if(m==="customers"){if(t.method==="GET"){const h=St(t.query.phone);if(!h)return e.status(400).json({error:"phone is required"});const y=await r`
        ...
        `;if(!D.length)return e.status(200).json({exists:!1});const W={name:D[0].customer_name||null,phone:h,address:D[0].address||null};try{const I=Nt.randomUUID(),O=await r`
          ...
          `;return e.status(200).json({exists:!0,customer:O[0]})}catch{const I=await r`
            ...
            WHERE phone = ${h}
            LIMIT 1
          `;return I.length?e.status(200).json({exists:!0,customer:I[0]}):e.status(200).json({exists:!1})}}return e.status(405).json({error:"Method not allowed"})}if(t.method==="GET"){let h=null;if(s){const i=Date.now();await Xt(),h=Date.now()-i}else await Xt();if(m==="stats"){const i=String(t.query.month??"").trim(),d=Wt(i);if(!d)return e.status(400).json({error:"month is required (YYYY-MM)"});const P=me(t.query.ship_percent??t.query.shipPercent),st=Tt(t.query.meta,!1);let c=[];try{c=await r`SELECT id, name, code, price, note, commission_percent, variants FROM products`}catch{c=[]}const f=new Map;for(const o of Array.isArray(c)?c:[]){const a=String(o?.id??"").trim();a&&f.set(a,o)}const et=o=>(Array.isArray(o)?o:[]).map(L=>{const U=String(L?.name||"").trim();if(!U)return null;const v=(Array.isArray(L?.options)?L.options:[]).map(R=>{const g=String(R?.label||"").trim();if(!g)return null;const S=R?.price,C=S==null||S===""?NaN:typeof S=="number"?S:typeof S=="string"?Ht(S):Number(S),V=Number.isFinite(C)?Math.trunc(C):null,Y=R?.price_delta??R?.priceDelta??null,$=Number(Y),J=Number.isFinite($)?Math.trunc($):0;return{label:g,price:V,price_delta:J}}).filter(Boolean);return{name:U,options:v}}).filter(Boolean),ct=o=>{const a=String(o??"").trim();return a&&f.get(a)||null},Bt=o=>{const a=o?.unit_price??o?.unitPrice,L=a==null||a===""?NaN:Number(a);if(Number.isFinite(L))return Math.max(0,Math.trunc(L));const U=String(o?.product_id||"").trim(),v=ct(U),R=Ht(v?.price),g=o?.variant_json??o?.variantJson,S=g&&typeof g=="object"?g:null;if(!S)return R;const C=et(v?.variants);if(!C.length)return R;let V=R;for(const Y of C){const $=String(Y?.name||"").trim(),J=String(S?.[$]||"").trim();if(!$||!J)continue;const at=(Array.isArray(Y.options)?Y.options:[]).find(Et=>String(Et?.label||"").trim()===J);if(!at)continue;if(Number.isFinite(Number(at.price))){V=Math.max(0,Math.trunc(Number(at.price)));continue}const mt=Number(at.price_delta);Number.isFinite(mt)&&(V+=Math.trunc(mt))}return V},At=5,Dt=new Map((Array.isArray(c)?c:[]).map(o=>{const a=o?.commission_percent??o?.commissionPercent,L=Number(a),U=Number.isFinite(L)?Math.max(0,Math.min(100,L)):At;return[String(o?.id),U]})),Vt=o=>o.customer_id||o.phone||"unknown",Yt=o=>{const a=Array.isArray(o)?o:[];let L=!1,U=0;for(const v of a){const R=String(v?.product_id||"").trim(),g=ct(R),S=v?.product_note??g?.note??null,C=de(S);C!=null&&(L=!0,C>U&&(U=C))}return{found:L,fee:U}},Gt=s?Date.now():0,Ft=await r(`
          SELECT
            o.id,
            o.parent_order_id,
            o.split_seq,
          ...
          ORDER BY o.created_at DESC
        `,[d]),$t=s?Date.now():0,wt=(Array.isArray(Ft)?Ft:[]).map(Ct),B={draft:0,pending:0,processing:0,done:0,paid:0,canceled:0,other:0};let it=0,ht=0,Ot=0,ft=0,Jt=0,Mt=0,jt=0,It=0;const yt=new Map,_t=new Map,Lt=new Map;for(const o of wt){const a=Ee(o?.status),v=a==="canceled"||a==="draft",R=Array.isArray(o?.items)?o.items:[];let g=0,S=0,C=0;for(const K of R){const w=Number(K?.quantity||0)||0,rt=Bt(K),nt=w*rt,G=String(K?.product_id||""),oe=Dt.has(G)?Dt.get(G):At,se=(Number(oe)||0)/100;if(g+=w,S+=nt,C+=nt*se,!v){const pt=K?.product_id||"unknown",bt=ct(pt),lt=yt.get(pt)||{product_id:pt,product_name:bt?.name||"\u2014",product_code:bt?.code||"",orders:0,quantity:0,revenue:0};lt.orders+=1,lt.quantity+=w,lt.revenue+=nt,yt.set(pt,lt)}}const V=Yt(R),Y=Number(o?.adjustment_amount??0)||0,$=S+(V.found?V.fee:0)+Y,J=S+Y,at=!V.found&&P>0&&S>0?S*P/100:0,mt=S>0?C/S:At/100,Et=C+Y*mt-at*mt,ne=a==="done"||a==="paid";if(a==="draft"?B.draft+=1:a==="pending"?B.pending+=1:a==="processing"?B.processing+=1:a==="canceled"?B.canceled+=1:a==="paid"?(B.paid+=1,B.done+=1,ft+=$,Mt+=J,It+=Et):a==="done"?(B.done+=1,ft+=$,Mt+=J,It+=Et):B.other+=1,!v){it+=1,ht+=g,Ot+=$,Jt+=J,jt+=Et;const K=Vt(o),w=_t.get(K)||{key:K,customer_name:o.customer_name||"",phone:o.phone||"",orders:0,quantity:0,revenue:0};w.orders+=1,w.quantity+=g,w.revenue+=$,!w.customer_name&&o.customer_name&&(w.customer_name=o.customer_name),!w.phone&&o.phone&&(w.phone=o.phone),_t.set(K,w);const rt=o.created_at?new Date(o.created_at):null;if(rt&&!Number.isNaN(rt.getTime())){const nt=`${rt.getFullYear()}-${String(rt.getMonth()+1).padStart(2,"0")}-${String(rt.getDate()).padStart(2,"0")}`,G=Lt.get(nt)||{day:nt,orders:0,quantity:0,revenue:0,doneOrders:0,doneRevenue:0};G.orders+=1,G.quantity+=g,G.revenue+=$,ne&&(G.doneOrders+=1,G.doneRevenue+=$),Lt.set(nt,G)}}}const Kt=Array.from(yt.values()).sort((o,a)=>a.revenue-o.revenue),zt=Array.from(_t.values()).sort((o,a)=>a.revenue-o.revenue),kt=Array.from(Lt.values()).sort((o,a)=>o.day.localeCompare(a.day)),Qt=_t.size,Zt=it?Math.round(Ot/it):0,te=it?ht/it:0,ee=Math.round(It),re=Math.round(jt),qt={statusCounts:B,activeOrders:it,totalQty:ht,totalRevenue:Ot,doneRevenue:ft,tempCommission:ee,tempCommissionAll:re,products:Kt,customers:zt,days:kt,uniqueCustomers:Qt,avgOrderValue:Zt,avgQtyPerOrder:te};return st?e.status(200).json({stats:qt,meta:{month:i,ship_percent:P,ordersCount:wt.length,...s?{timingsMs:{schema:M-N,cleanup:ot-M,autoAdvance:h,query:$t-Gt,total:$t-N}}:{}}}):e.status(200).json(qt)}if(n){const d=await r(`
          SELECT
            o.id,
            o.parent_order_id,
            o.split_seq,
