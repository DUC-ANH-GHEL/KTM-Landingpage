...
export default async function handler(req, res) {
  ...
  const pathId = Array.isArray(pathParam) && pathParam.length ? pathParam[0] : pathParam;
  const id = pathId || req.query.id || req.body?.id;
  ...
  try {
    ...
    if (req.method === 'POST') {
      if (pathId) {
        return res.status(400).json({ error: 'Use /api/orders for creating orders' });
      }
